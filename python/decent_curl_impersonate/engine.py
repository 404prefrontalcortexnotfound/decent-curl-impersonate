...
class CurlEngine:
    ...
    async def dispatch(self, operation: str, params: dict[str, Any]) -> dict[str, Any]:
        async with self._state_lock:
            if self._closed:
                raise EngineError("engine_closed", "Curl engine is closed")
    ...
    async def reap_idle(self) -> None:
        """Close inactive named sessions and WebSockets after the configured limit."""
        if self._idle_timeout is None:
            return
        deadline = self._clock() - self._idle_timeout
        async with self._state_lock:
            websockets: list[_WebSocketHandle] = []
            for websocket_id, handle in tuple(self._websockets.items()):
            ...
            websocket_sessions = {
                handle.session_id
                for handle in self._websockets.values()
                if handle.session_id is not None
            }
            sessions: list[_NamedSession] = []
            for session_id, named in tuple(self._sessions.items()):
                if (
                    named.active_uses == 0
                    and named.last_used <= deadline
                    and session_id not in websocket_sessions
                ):
    ...
    async def run_idle_cleanup(self, interval: float) -> None:
        """Reap inactive resources until the caller cancels this task."""
        if interval <= 0:
            raise ValueError("interval must be a positive number of seconds")
        while True:
            await asyncio.sleep(interval)
            sweep = asyncio.create_task(self.reap_idle())
            try:
    ...
    async def _session_create(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._state_lock:
            if self._closed:
                raise EngineError("engine_closed", "Curl engine is closed")
            self._sessions[session_id] = _NamedSession(
                AsyncSession(), profile, self._clock()
            )
    ...
    async def _websocket_connect(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._use_session(params) as selected:
            session, should_close, default_profile, session_id = selected
            ...
            async with self._state_lock:
                session_is_open = (
                    session_id is None
                    or (
                        (named := self._sessions.get(session_id)) is not None
                        and named.session is session
                    )
                )
                register_handle = not self._closed and session_is_open
                if register_handle:
            ...
            if not register_handle:
                try:
                    await websocket.close()
                finally:
                    if should_close:
                        await session.close()
                code = "engine_closed" if self._closed else "unknown_session"
                message = (
                    "Curl engine is closed"
                    if self._closed
    ...
    async def _websocket_send(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if has_text:
            ...
            message_type = "binary"
        async with self._use_websocket(params) as (websocket_id, handle):
            try:
                await handle.websocket.send(payload, flags=flags, timeout=timeout)
            except asyncio.TimeoutError:
                raise EngineError(
                    "timeout", "WebSocket send timed out", retryable=True
                ) from None
            except WebSocketError as error:
                if error.code != CurlECode.OPERATION_TIMEDOUT:
                    await self._retire_terminal_websocket(websocket_id, handle)
                raise self._network_error(error) from None
        return {
    ...
    async def _websocket_close(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._state_lock:
            handle = self._websockets.pop(websocket_id, None)
            if handle is None:
    ...
    async def _request_execute(self, params: dict[str, Any]) -> dict[str, Any]:
        async with self._use_session(params) as selected:
            session, should_close, default_profile, _ = selected
            try:
                profile = self._profile(params.get("profile", default_profile))
    ...
    async def _use_session(
    ...
    ) -> AsyncIterator[tuple[AsyncSession, bool, str | None, str | None]]:
        session_id = params.get("session_id")
        if session_id is None:
            yield AsyncSession(), True, None, None
            return
        ...
        async with self._state_lock:
            ...
            if named is None:
                raise EngineError("unknown_session", "Unknown session ID")
            named.active_uses += 1
            named.idle_event.clear()
        try:
            yield named.session, False, named.profile, session_id
        finally:
            async with self._state_lock:
                named.active_uses -= 1
                named.last_used = self._clock()
                if named.active_uses == 0:
                    named.idle_event.set()

    @asynccontextmanager
    async def _use_websocket(
        self, params: dict[str, Any]
    ) -> AsyncIterator[tuple[str, _WebSocketHandle]]:
        websocket_id = self._required_string(params, "websocket_id")
        async with self._state_lock:
            ...
            handle.idle_event.clear()
        try:
            yield websocket_id, handle
        finally:
            close_handle = False
            async with self._state_lock:
                handle.active_uses -= 1
                ...
                if handle.active_uses == 0:
                    handle.idle_event.set()
                    close_handle = handle.retired
            if close_handle:
                await self._close_websocket_once(handle)
